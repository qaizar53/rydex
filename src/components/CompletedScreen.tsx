        ...
        >
          ...
          <p className="text-zinc-500 text-sm text-center mb-8">
            {role === "driver"
              ...
              : "You have successfully dropped by the driver"}
          </p>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 mb-3">
            ...
            <div className="flex items-center justify-between text-xs border-t border-zinc-800 pt-3 gap-4">
              <span className="text-zinc-500">Payment Status</span>

              <span
                className={`px-2.5 py-1 rounded-full font-semibold text-[11px] ${PAYMENT_BADGE[bookings.paymentStatus]?.cls ?? "bg-zinc-700 text-zinc-300"}`}
              >
                {PAYMENT_BADGE[bookings.paymentStatus]?.label ??
          ...
          {bookings?.user && (
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex items-center gap-3 mb-3">
              ...
              <div>
                <p className="text-zinc-500 text-[10px] uppercase tracking-widest font-semibold">Customer</p>

                <p className=" text-white text-sm font-semibold">
                    {bookings?.user?.name ?? "user"}
                </p>
              </div>
          ...
          <button className="w-full border border-zinc-700 text-zinc-400 py-3.5 rounded-2xl text-sm font-semibold hover:bg-zinc-900 transition-colors" onClick={()=>router.push("/")}>
            Back To Home
          </button>
